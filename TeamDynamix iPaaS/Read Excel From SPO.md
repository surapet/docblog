...
Get a row from a Table in an Excel Workbook  
/drives/{drive-id}/items/{item-id}/workbook/worksheets/{worksheet-id}/tables/{table-id}/rows/itemAt(index={table-index})/range  
Get the range of cells used in an Excel Workbook.  
Note: This information is also given in most all the other funtions when getting a Workbook.  
/drives/{drive-id}/items/{item-id}/workbook/worksheets/{worksheet-id}/usedRange
